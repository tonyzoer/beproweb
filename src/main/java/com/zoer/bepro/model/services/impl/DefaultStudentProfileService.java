...
public class DefaultStudentProfileService extends GenericEntityService<StudentProfile> implements StudentProfileService {
    private static final  Logger logger = Logger.getLogger(DefaultStudentProfileService.class);
    ...
    public boolean addSpecification(Specifications spec, StudentProfile sp) {
        try {
            ...
            logger.debug("Specification id :"+spec.getValue()+" wasnt added to student profile id:"+sp.getId()+"with exception "+ e);
            return false;
        }
        return true;
    }
    ...
    public boolean addSpecification(String spec, StudentProfile sp) {
        ...
        try {
            newSpec = DefaultServiceFactory.getInstance().getDefaultSpecificationService().insert(new Specifications() {{
                setValue(spec);
            }});
            addSpecificationImp(newSpec, sp);
        } catch (PersistException e) {
            logger.debug("Specification id :"+spec+" wasnt added to student profile id:"+sp.getId()+"with exception "+ e);
            return false;
        }
        return true;
    }
